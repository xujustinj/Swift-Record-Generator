...
func dictionarySorting(by element: Element, mutable: Bool) -> String {
    ...
    if element.isOptional {
        if element.wrapper == .none {
            criterion = """
            guard let lhs = $0.\(element.getter)(key: \(element.key)), let rhs = $1.\(element.getter)(key: \(element.key)) else { return false }
                    return lhs < rhs
            """
        } else {
            criterion = "$0.\(element.getter)(key: \(element.key))?.count ?? 0 < $1.\(element.getter)(key: \(element.key))?.count ?? 0"
        }
    } else {
        if element.wrapper == .none {
            criterion = "$0.\(element.getter)(key: \(element.key)) < $1.\(element.getter)(key: \(element.key))"
        } else {
